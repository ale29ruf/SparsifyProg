...
Sparsify è una libreria Python per l'addestramento di Sparse Autoencoders (SAE) su modelli di linguaggio. La libreria permette di:

- Addestrare SAE su specifici punti (hookpoints) di un modello
- Gestire l'addestramento distribuito
- Supportare diversi ottimizzatori (Adam, Muon, SignSGD)
- Implementare tecniche di sparsificazione avanzate
...
```python
from sparsify import Trainer, TrainConfig
...
cfg = TrainConfig(
    hookpoints=["model.layers.0.mlp"],  # Punti del modello da monitorare
    batch_size=32,
    lr=2e-4
)
...
I principali parametri di configurazione includono:

- `hookpoints`: Punti del modello da monitorare
- `batch_size`: Dimensione del batch
- `lr`: Learning rate
- `sae.k`: Numero di feature latenti attive
- `optimizer`: Tipo di ottimizzatore ("adam", "muon", "signum")

## Documentazione

Per la documentazione completa, consulta il file `CHANGELOG.md` e i docstring nel codice.
